...
// Fetch order cart details if an order ID is selected
if ($selected_order_id) {
    // Adjust query to match your table structure
    $sql_order_cart = "
        SELECT p.product_name, c.quantity, c.price, c.total
        FROM order_cart c
        JOIN products p ON c.product_id = p.id
        WHERE c.order_id = ?";
    $stmt_order_cart = $conn->prepare($sql_order_cart);
    if ($stmt_order_cart === false) {
        die('Prepare failed: ' . htmlspecialchars($conn->error));
    }
    $stmt_order_cart->bind_param("i", $selected_order_id);
    $stmt_order_cart->execute();
    $order_cart_result = $stmt_order_cart->get_result();
}
...
if ($selected_order_id && isset($stmt_order_cart)) {
    $stmt_order_cart->close();
}
...
<body>
    <div class="container table-container">
        ...
        <?php if ($selected_order_id) { ?>
            ...
            <?php if (isset($order_cart_result) && $order_cart_result->num_rows > 0) { ?>
                <table class="table table-striped order-cart-table">
                    <thead>
                        <tr>
                            <th>Product Name</th>
                            <th>Quantity</th>
                    ...
                    <tbody>
                        <?php while ($item = $order_cart_result->fetch_assoc()) { ?>
                            <tr>
