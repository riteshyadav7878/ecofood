...
<head>
    ...
    <style>
        .error {
            border: 2px solid red !important; /* Add red border for invalid fields */
        }
    </style>
</head>
<body>
    ...
    <script>
        document.getElementById('registerForm').addEventListener('submit', function(event) {
            ...
            inputs.forEach(input => {
                if (!input.value.trim()) {
                    input.classList.add('error'); // Add red border for empty fields
                    isValid = false;
                } else {
                    input.classList.remove('error'); // Remove red border if field is valid
                }
            });
        ...
        document.querySelectorAll('input[required], textarea[required]').forEach(input => {
            input.addEventListener('input', function() {
                if (this.value.trim()) {
                    this.classList.remove('error'); // Remove red border when user starts typing
                } else {
                    this.classList.add('error'); // Add red border back if input is still empty
                }
            });
        });
    </script>
