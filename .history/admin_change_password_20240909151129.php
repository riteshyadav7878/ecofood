...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_details']) && $show_change_form) {
    ...
    if (!isset($_SESSION['admin_id'])) {
        ...
        $stmt = $conn->prepare("SELECT * FROM admin WHERE admin_id = ?"); // Ensure column name matches
        ...
        if ($result->num_rows > 0) {
            ...
            if (!password_verify($current_password, $row['password'])) {
                ...
                $stmt = $conn->prepare("UPDATE admin SET username = ?, password = ? WHERE admin_id = ?"); // Ensure column name matches
