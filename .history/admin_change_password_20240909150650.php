...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['login_admin'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $row['password'])) {
            ...
            $_SESSION['admin_id'] = $row['admin_id']; // Updated column name
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_details']) && $show_change_form) {
    ...
    $stmt = $conn->prepare("SELECT * FROM admin WHERE admin_id = ?"); // Updated column name
    ...
    if (!password_verify($current_password, $row['password'])) {
        ...
        $stmt = $conn->prepare("UPDATE admin SET username = ?, password = ? WHERE admin_id = ?"); // Updated column name
