...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['add_product'])) {
        ...
        $sql = "INSERT INTO products (title, price, image, status) VALUES ('$title', '$price', '$image', '$status')";
    ...
    if (isset($_POST['update_product'])) {
        ...
        if (!empty($image)) {
            move_uploaded_file($image_tmp, "images/$image");
            $sql = "UPDATE products SET title='$title', price='$price', image='$image', status='$status' WHERE id='$id'";
        } else {
            // If no new image, don't change the image in the database
            $sql = "UPDATE products SET title='$title', price='$price', status='$status' WHERE id='$id'";
        }
        mysqli_query($conn, $sql);
    }
