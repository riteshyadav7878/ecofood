    ...
    if (isset($_POST['update_blog'])) {
        ...
        if ($_FILES['image']['error'] == 0) {
            ...
            if (move_uploaded_file($image_tmp_name, $image_path)) {
                $sql = "UPDATE blog_posts SET title='$title', description='$description', image='$image_name' WHERE id='$blog_id'";
            } else {
                $error = "Image upload failed.";
            }
        } else {
            $sql = "UPDATE blog_posts SET title='$title', description='$description' WHERE id='$blog_id'";
        }
        ...
        if ($_FILES['image']['error'] == 0) {
            ...
            if (move_uploaded_file($image_tmp_name, $image_path)) {
                $sql = "INSERT INTO blog_posts (title, description, image) VALUES ('$title', '$description', '$image_name')";
            } else {
                $error = "Image upload failed.";
            }
        } else {
...
if (isset($_GET['delete_blog'])) {
    ...
    $sql = "SELECT image FROM blog_posts WHERE id='$blog_id'";
...
$sql = "SELECT id, title, description, image, created_at FROM blog_posts ORDER BY created_at DESC";
$result = mysqli_query($conn, $sql);
?>
            ...
            <tbody>
                <?php while ($row = mysqli_fetch_assoc($result)): ?>
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-primary" data-toggle="modal" data-target="#editModal" data-id="<?php echo htmlspecialchars($row['id']); ?>" data-title="<?php echo htmlspecialchars($row['title']); ?>" data-description="<?php echo htmlspecialchars($row['description']); ?>" data-image="<?php echo htmlspecialchars($row['image']); ?>">Edit</button>
