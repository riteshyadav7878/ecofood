...
// Check if the admin_id is set in the session before querying
if (isset($_SESSION['admin_id'])) {
    $admin_id = intval($_SESSION['admin_id']); // Safely cast admin_id to an integer

    // SQL query to check if the admin still exists
    $sql = "SELECT id FROM user WHERE id = $admin_id";
    $result = $conn->query($sql);

    if ($result->num_rows == 0) {
        // Admin no longer exists, destroy the session and redirect to login page
        session_destroy();
        header("Location: admin_login.php");
        exit();
    }
} else {
    // If no admin_id in session, redirect to login page
    session_destroy();
    header("Location: admin_login.php");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['delete_user'])) {
        ...
        if ($conn->query($sql) === TRUE) {
            // If the deleted user is the current logged-in admin, destroy their session
